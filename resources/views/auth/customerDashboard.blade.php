        ...
        <a href="{{ route('dashboard.store.view', ['id' => $store->id]) }}"
           ...
           data-name="{{ strtolower($store->name) }}">
            ...
            <div class="relative h-48 overflow-hidden">
                @if($store->logo)
                    <img src="{{ $store->logo }}" alt="{{ $store->name }} Logo" class="w-full h-full object-cover">
                @else
                    <div class="w-full h-full bg-gradient-to-br from-{{ $index % 2 == 0 ? 'green' : 'blue' }}-400 to-{{ $index % 2 == 0 ? 'blue' : 'green' }}-600 flex items-center justify-center">
